...
const STORAGE_KEY = 'guesthouse_calendar_v3';
let state = loadState();
...
document.getElementById('exportAllBtn').onclick = ()=>{
  ...
  for(const room of ROOMS){
    for(const b of getBookings(room)){
      rows.push({room,start:b.start,end:b.end,name:b.name||'',note:b.note||''});
    }
  }
...
document.getElementById('exportMonthBtn').onclick = ()=>{
  ...
  for(const room of ROOMS){
    for(const b of getBookings(room)){
      if(!(b.end < monthStart || b.start > monthEnd)){
        rows.push({room,start:b.start,end:b.end,name:b.name||'',note:b.note||''});
      }
    }
  }
    ...
    for(const day of cells){
      ...
      if(!outside){
        ...
        if(state.tentative && state.tentative.room===room){
          const [s,e] = clampRange(state.tentative.start, dISO);
          if(dISO>=s && dISO<=e) el.classList.add('tent');
        }
...
let currentCtx = null; // {mode:'create'|'edit', room, booking}

function openSheet(mode, room, booking){
  currentCtx = {mode, room, booking};
  ...
  deleteBtn.onclick = ()=>{
    if(mode!=='edit') return;
    const list = getBookings(room);
    const idx = list.findIndex(x=>x.id===booking.id);
    if(idx>=0){ list.splice(idx,1); }
    state.tentative=null; save(); closeSheet(); paint();
  };

  closeBtn.onclick = ()=>{ state.tentative=null; closeSheet(); };
  ...
  form.onsubmit = (e)=>{
    ...
    if(!s || !eDate) return;
    const [ss,ee] = clampRange(s,eDate);
    const list = getBookings(room);
    if(overlapsAny(list, ss, ee, mode==='edit'?booking.id:null)){
      alert('Those dates overlap another booking.'); return;
    }
    if(mode==='create'){
