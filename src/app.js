...
signinBtn.onclick = async ()=>{
  try { await signIn(emailEl.value.trim(), passEl.value.trim()); }
  catch (e) { alert(e.message || 'Sign-in failed'); }
};
signupBtn.onclick = async ()=>{
  try { await createAccount(emailEl.value.trim(), passEl.value.trim()); alert('Account created — you can sign in now.'); }
...
function closeSheet(){
  sheet.classList.remove('open'); backdrop.classList.remove('open'); sheet.setAttribute('aria-hidden','true');
  document.body.style.overflow='auto'; currentEdit=null;
}
...
deleteBtn.onclick = async ()=>{
  try{
    if(!currentEdit || !isAdmin) return alert('You are read-only.');
    await deleteDoc(doc(db, "bookings", currentEdit.booking.id));
  }catch(e){
...
form.onsubmit = async (e)=>{
  ...
  const payload={ room: currentEdit.room, start:ss, end:ee, name:nameInput.value||'', note:noteInput.value||'', updatedAt: serverTimestamp(), createdBy: (auth.currentUser && (auth.currentUser.email||auth.currentUser.uid)) || '' };
  try{
    if(currentEdit.mode==='edit'){
      if(!isAdmin) return alert('You are read-only.');
      await updateDoc(doc(db,"bookings", currentEdit.booking.id), payload);
    } else {
      if(!isAdmin) return alert('You are read-only.');
