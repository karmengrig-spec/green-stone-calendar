...
// Rooms with your exact names
const ROOMS = ['Double','Twin','Deluxe','Standard','Family','Cottage','Sauna'];

const STORAGE_KEY = 'guesthouse_calendar_v5';
let state = loadState();
...
document.getElementById('exportMonthBtn').onclick = ()=>{
  const d=parseISO(state.view); const m=d.getMonth()+1, y=d.getFullYear();
...
function paint(){
  ...
  for(const room of ROOMS){
    ...
    for(const day of cells){
      ...
      if(!outside){
        el.classList.add('free'); el.textContent=day.getDate();
        const dISO=iso(day);
...
function onDayTap(room, date){
  ...
  // Create flow: clicking a green (free) date opens the sheet with that date prefilled
...
function openSheet(mode, room, booking){
  ...
  form.onsubmit = (e)=>{
    ...
    const list = getBookings(room);
    const exclude = mode==='edit' ? booking.id : null;
    if(overlapsAny(list, ss, ee, exclude)){ alert('Those dates overlap another booking.'); return; }
    if(mode==='create'){
